...
void Denoiser::SetPixelMeanAndVarianceFinal(const vector<SubPixel> &subPixels,
    ...
    if (PbrtOptions.verbose && !PbrtOptions.quiet) {
        DumpErrorMap(DUMP_FINAL);
        DumpImageRGB(_imgVar, "var", DUMP_FINAL);
        DumpDecomposition(_decAvg, "img", DUMP_FINAL);
        DumpDecomposition(_decVar, "var", DUMP_FINAL);
        DumpDecomposition(_decMse, "mse", DUMP_FINAL);
        DumpDecomposition(_decSel, "sel", DUMP_FINAL);
    }
