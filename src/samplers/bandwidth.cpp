...
void BandwidthSampler::MarkAdaptPixels(const std::vector<float >& w_ij)
{
    struct Comp{
    ...
    std::sort(vx.begin(), vx.end(), Comp(w_ij));
	...
	seen.insert(std::pair<int, int>(xPos, yPos));
        ...
        _pixels[seen.size()-1]._yPos = yPos + .5f;
...
BandwidthSampler *CreateBandwidthSampler(const ParamSet &params,
    ...
    int howmany = params.FindOneInt("ErrorPixelsPerIteration", 100);
